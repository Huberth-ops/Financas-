...
export default function App() {
  ...
  function copyFromMonth() {
    ...
    if (!src) return;
    const copied = { salary: monthData.salary, expenses: src.expenses.map(e => ({ ...e, paid: false, id: Date.now().toString() + Math.random() })) };
  ...
  const annualData = useMemo(() => {
    return MONTHS_SHORT.map((label, i) => {
      ...
      const d = allMonths[k];
      if (!d || !d.salary) return { label, salary: 0, expenses: 0, balance: 0, hasData: false };
      const exp = (d.expenses || []).reduce((s, e) => s + Number(e.value), 0);
      return { label, salary: Number(d.salary), expenses: exp, balance: Number(d.salary) - exp, hasData: true };
    });
  }, [allMonths, selYear]);
      ...
      {tab === "contas" && (<>
        <div className="card">
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14}}>
            <span style={{fontSize:11,color:"#a8a29e",textTransform:"uppercase",letterSpacing:"1.2px",whiteSpace:"nowrap",fontWeight:700}}>Salário</span>
            <input className="field" type="number" value={monthData?.salary||""} placeholder="0,00" onChange={e => updateSalary(e.target.value)} style={{fontWeight:700,fontSize:15}} />
          </div>
          {expenses.length > 0 && (<>
            <div style={{display:"flex",justifyContent:"space-between",fontSize:11,color:"#a8a29e",marginBottom:5,fontWeight:500}}>
